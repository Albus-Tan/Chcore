...
s32 wait_sem(struct semaphore *sem, bool is_block)
{
        ...
        if(sem->sem_count == 0){
                if(is_block){
                        // update the state of current thread to TS_WAITING
                        ...
                        // `obj_put` the sem before eret to the new thread
                        obj_put(sem);

                        // use sched() and eret_to_thread if current thread should be blocked
                        sched();
...
s32 signal_sem(struct semaphore *sem)
{
        ...
        if(sem->waiting_threads_count > 0){
                struct thread *to_awake = list_entry(sem->waiting_threads.next, struct thread, sem_queue_node);
                list_del(&to_awake->sem_queue_node);
                --sem->waiting_threads_count;
                sched_enqueue(to_awake);
        } else {
                ++sem->sem_count;
        }
        /* LAB 4 TODO END */
