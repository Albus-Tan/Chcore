	...
	switch(fr->req) {
		...
		/* LAB 5 TODO BEGIN */
                case FS_REQ_OPEN:
                {
                        ...
                        struct ipc_msg *ipc_msg_forward = ipc_create_msg(
                                mpinfo->_fs_ipc_struct, sizeof(struct fs_request), 0);
                        chcore_assert(ipc_msg_forward);
                        struct fs_request * fr_forward = (struct fs_request *)ipc_get_msg_data(ipc_msg_forward);
                        ...
                        int ret = ipc_call(mpinfo->_fs_ipc_struct, ipc_msg_forward);
